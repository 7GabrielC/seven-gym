...
export default async function NuevoPagoPage() {
    ...
    const listaSocios = await db
        ...
        .leftJoin(
        suscripciones,
        and(eq(suscripciones.socioId, socios.id), isNull(suscripciones.eliminadoEn))
        )
    ...
    for (const p of precios) {
        if (!precioPorPlan.has(p.planId)) precioPorPlan.set(p.planId, p.precioCentavos);
    }
    ...
    return (
        ...
        <h1 className="text-2xl font-semibold tracking-tight mb-1">Cobrar cuota</h1>
