...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
    ...
    const [cajaAbierta] = await db
        ...
        .from(sesionesCaja)
        .where(and(eq(sesionesCaja.usuarioId, session.user.id), isNull(sesionesCaja.cerradaEn)))
        .limit(1);
    ...
    return (
        ...
        <div className="flex-1 min-w-0 flex flex-col">
            ...
            {cajaAbierta ? (
                <Link href="/caja" className="flex items-center gap-2 group" title="Ir a la caja">
            ...
            <div className="flex items-center gap-4">
                <span className="text-xs text-muted-foreground capitalize">{fecha}</span>
                <BotonTema />
            </div>
            </header>
