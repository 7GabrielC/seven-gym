...
export default async function SociosBajaPage() {
    ...
    return (
        <div className="max-w-4xl px-8 py-7">
            ...
            <h1 className="text-2xl font-semibold tracking-tight mb-1">Dados de baja</h1>
            <p className="text-sm text-muted-foreground mb-6">
                Se reactivan automáticamente al intentar crear un socio con el mismo DNI.
            </p>

            {dados.length === 0 ? (
                <p className="text-sm text-muted-foreground">No hay socios dados de baja.</p>
            ) : (
                <div className="rounded-lg border border-border bg-card overflow-hidden">
                    <table className="w-full">
                        ...
                        {dados.map((s) => (
                            ...
                            <td className="py-2.5 px-4 text-sm text-muted-foreground tabular">
                                {s.eliminadoEn && new Date(s.eliminadoEn).toLocaleDateString("es-AR")}
                            </td>
                            </tr>
                        ))}
                        </tbody>
