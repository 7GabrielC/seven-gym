...
export function FormSocio() {
  ...
  const [socioBaja, setSocioBaja] = useState<{
    id: number;
    nombre: string;
  } | null>(null);
  ...
  async function manejarSubmit(formData: FormData) {
    ...
    if (resultado?.error === "dado_de_baja" && resultado.socioBajaId) {
      setSocioBaja({
        id: resultado.socioBajaId,
        nombre: resultado.nombreBaja!,
      });
      setCargando(false);
        ...
        <form action={manejarSubmit} className="space-y-4">
          ...
          {socioBaja && (
            <div className="rounded-md border border-warning/25 bg-warning-soft p-3.5">
              <p className="text-sm">
                Ese DNI pertenece a <strong>{socioBaja.nombre}</strong>, un
                socio dado de baja. Podés reactivarlo en vez de crear uno nuevo.
              </p>
              <Button
                type="button"
                size="sm"
