import { pgTable, bigserial, varchar, integer, timestamp, date, bigint, boolean, text, pgEnum, index } from "drizzle-orm/pg-core";
...
export const observaciones = pgTable("observaciones", {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    socioId: bigint("socio_id", { mode: "number" }).notNull().references(() => socios.id),
    autorId: text("autor_id").notNull().references(() => user.id),
